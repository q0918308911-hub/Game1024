...
var MyLib = (() => {
  ...
  ((GameConfig0552) => {
    ...
    GameConfig0552.TEST_DATA = [
      ""
    ];
  ...
  var MegaWaysWinScoreAnalyzer = class {
    ...
    calculateOneIconMegaWays(icon, icon2DData) {
      ...
      for (let i = 0; i < this.reelAmount; i++) {
        let hasWild = this.wild.some((value) => icon2DData[i].includes(value));
      ...
      if (!this.wild.includes(icon)) {
        const wildPos = this.getWildPos(icon2DData.flat(), 0);
    ...
    */
    getOneIconWinData(icon, expandedWinPaths) {
      if (expandedWinPaths.length !== 0) {
        const tempCount = expandedWinPaths[0].length - 1;
        const targetOdds = this.payTable[icon][tempCount];
  ...
  var RoundModel055 = class {
    ...
    calculateFreeSpin() {
      ...
      if (!this.isFG) {
        ...
        const addCount = extraLength - MAX_COMBO + 1;
        if (addCount > 0) {
    ...
    getReadyHandReelIDList(diskData) {
      ...
      for (let index = 0; index < diskData.length; index++) {
        ...
        const reelScatterCount = reelData.filter((symbolID) => symbolID === SCATTER_SYMBOL).length;
    ...
    getExtraReadyHandReelIDList(diskData, extraDataList) {
      ...
      let readyHandReelIDList = [];
      if (scatterCount === SCATTER_STANDARD - 1) {
        for (let index = 0; index < extraDataList.length; index++) {
          const extraData = extraDataList[index];
          if (scatterCount === SCATTER_STANDARD - 1) {
            readyHandReelIDList.push(index);
          }
  ...
  function historyParser(base64Str, bet) {
    ...
    let slotInfo055 = new SlotInfo055(binaryBuffer, bet);
